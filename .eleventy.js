...
module.exports = function(config) {
  ...
  config.addFilter("jsmin", code => {
    let mini = require('terser').minify(code)
    if (mini.error) {
      console.error("[JS Minifier error]: ", mini.error)
      return code
    }
    return mini.code
  })
  ...
  // custom filters
  config.addFilter("navigation", nav => {
    return nav.filter(item => item.url.indexOf('/tag/') !== -1)
      .map(item => ({
        name: item.label,
        slug: item.url.replace('/tag/', '').replace(/\/$/, '')
      }))
  })

  config.addFilter("filterByTag", (collection, tag) => {
    return collection
      .filter(item => item.primary_tag && item.primary_tag.slug === tag)
  })
